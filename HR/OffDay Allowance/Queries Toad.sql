...
WHERE
H.COMPANY_CODE =:P_COMPANY_CODE 
AND H.COMPANY_BRANCH_CODE =:P_COMPANY_BRANCH_CODE 
...
COMPANY_CODE        = :P_COMPANY_CODE AND
COMPANY_BRANCH_CODE = :P_COMPANY_BRANCH_CODE AND
TO_CHAR(HOLIDAY_DATE,'MM/YYYY') = TO_CHAR(ADD_MONTHS(TO_DATE(:P_MNYR, 'MM/YYYY'),-1), 'MM/YYYY')
UNION
SELECT DATE_TO HOLIDAY_DATE
FROM HOLIDAY
WHERE 
COMPANY_CODE        = :P_COMPANY_CODE AND
COMPANY_BRANCH_CODE = :P_COMPANY_BRANCH_CODE AND
TO_CHAR(FROM_DATE,'MM/YYYY')   = TO_CHAR(ADD_MONTHS(TO_DATE(:P_MNYR, 'MM/YYYY'),-1), 'MM/YYYY')
...
A.ATTEND_DATE IN 
        ...
        WHERE COMPANY_CODE = :P_COMPANY_CODE AND
        COMPANY_BRANCH_CODE = :P_COMPANY_BRANCH_CODE AND
        INIT_YR(TO_CHAR(HOLIDAY_DATE,'MM/YYYY')) = INIT_YR(TO_CHAR(ADD_MONTHS(TO_DATE(:P_MNYR, 'MM/YYYY'),-1), 'MM/YYYY'))
        UNION
        SELECT DATE_TO
        FROM HOLIDAY
        WHERE 
        COMPANY_CODE = :P_COMPANY_CODE AND
        COMPANY_BRANCH_CODE = :P_COMPANY_BRANCH_CODE AND
        INIT_YR(TO_CHAR(FROM_DATE,'MM/YYYY'))   = INIT_YR(TO_CHAR(ADD_MONTHS(TO_DATE(:P_MNYR, 'MM/YYYY'),-1), 'MM/YYYY'))
        ) AND
(A.EMP_ID, A.ATTEND_DATE) NOT IN (
        SELECT EMP_ID, ADJ_DATE
        FROM HR_OFFDAY_ADJUST
...
(B.EMP_STATUS IN('A') OR B.EMP_STATUS IN('I') AND 
B.EMP_INACTIVE_DT > (TO_DATE('01/'||TO_CHAR(ADD_MONTHS(TO_DATE(:P_MNYR, 'MM/YYYY'),-1), 'MM/YYYY'),'DD/MM/YYYY'))) AND
INIT_YR(TO_CHAR(A.ADJ_DATE ,'MM/YYYY')) = INIT_YR(TO_CHAR(ADD_MONTHS(TO_DATE(:P_MNYR, 'MM/YYYY'),-1), 'MM/YYYY'))   ) X))) Y
GROUP BY DEPARTMENT, SECTION_NAME, DEPT_GROUP, DEPT_CODE, DEPT_GROUP_SL)           
