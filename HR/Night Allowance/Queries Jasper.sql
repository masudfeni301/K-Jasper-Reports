...
--------------------------------------------Night Allowance Compare Sheet Start -----------------------------------------------------
SELECT 
...
AND O.EFFECTIVE_DATE  = (SELECT MAX(EFFECTIVE_DATE) 
                     ...
                     DEPT_CODE        = X.DEPT_CODE  AND
                     DESIG_CODE      = X.DESIG_CODE AND
                     EFFECTIVE_DATE  <= TO_DATE(TO_CHAR(ADD_MONTHS(TO_DATE($P{P_MNYR}, 'MM/YYYY'),-1), 'MM/YYYY'),'MM/YYYY'))) NIGHT_ALL
FROM (SELECT JOIN_DT,BANK_SAL_FLAG,EMP_AC_NO,EMP_AC_DATE,MOBILE_BANK_SAL_FLAG,MOBILE_AC_NO,MOBILE_AC_DATE,ATTEND_DATE ,
DD,EMP_ID,NAME,IN_TIME,OUT_TIME,BASIC_SALARY,DEPT_GROUP,DEPT_CODE,DESIG_CODE,DESIG_GROUP,SALARY_TYPE
FROM
(SELECT DISTINCT B.JOIN_DT,B.BANK_SAL_FLAG,B.EMP_AC_NO,B.EMP_AC_DATE,B.MOBILE_BANK_SAL_FLAG,B.MOBILE_AC_NO,B.MOBILE_AC_DATE, A.ATTEND_DATE ,
...
(A.EMP_ID ,A.ATTEND_DATE) NOT IN (
        ...
        COMPANY_CODE=$P{P_COMPANY_CODE} AND
        COMPANY_BRANCH_CODE=$P{P_COMPANY_BRANCH_CODE} AND
        INIT_YR(TO_CHAR(ADJ_DATE,'MM/YYYY')) =  INIT_YR(TO_CHAR(ADD_MONTHS(TO_DATE($P{P_MNYR}, 'MM/YYYY'),-1), 'MM/YYYY')) )  AND 
A.COMPANY_CODE                                  = $P{P_COMPANY_CODE}      AND
A.COMPANY_BRANCH_CODE                           = $P{P_COMPANY_BRANCH_CODE}      AND
(S.DESIG_GROUP                                  = $P{P_DESIG_GROUP} OR $P{P_DESIG_GROUP} IS NULL)     AND
(B.DEPT_GROUP                                   = $P{P_DEPT_GROUP} OR $P{P_DEPT_GROUP} IS NULL)       AND
(B.DEPT_CODE                                    = $P{P_DEPT_CODE} OR $P{P_DEPT_CODE} IS NULL)      AND
(A.EMP_ID                                       = $P{P_EMP} OR $P{P_EMP} IS NULL)         AND
TO_CHAR(A.ATTEND_DATE,'MM/YYYY') = TO_CHAR(ADD_MONTHS(TO_DATE($P{P_MNYR}, 'MM/YYYY'),-1), 'MM/YYYY')    
...
(B.DEPT_CODE                                    = $P{P_DEPT_CODE} OR $P{P_DEPT_CODE} IS NULL)      AND
(A.EMP_ID                                       = $P{P_EMP} OR $P{P_EMP} IS NULL)         AND
TO_CHAR(A.ADJ_DATE,'MM/YYYY') = TO_CHAR(ADD_MONTHS(TO_DATE($P{P_MNYR}, 'MM/YYYY'),-1), 'MM/YYYY')   )                               
ORDER BY 
ATTEND_DATE,
DEPT_CODE  ) X)
GROUP BY 
...
--------------------------------------------Night Allowance Compare Sheet End-----------------------------------------------------
