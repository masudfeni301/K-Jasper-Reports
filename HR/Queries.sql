...
SELECT NAME, DESIGNATION, DEPARTMENT, SECTION_NAME, COUNT(ATTEND_DATE) as OFFDAY_QTY, EMP_ID, DEPT_GROUP, DEPT_CODE, DEPT_GROUP_SL, JOIN_DT,  COUNT(OFFDAY_LUNCH_DIN_ALL) as LUNCH_DIN_QTY, SUM(TOTAL_PAY_AMT) AS TOT_PAY_AMT,
...
AND O.EFFECTIVE_DATE  = (SELECT MAX(EFFECTIVE_DATE) 
                     ...
                     EFFECTIVE_DATE  <= TO_DATE(:P_MNYR,'MM/YYYY'))) as OFFDAY_ALL,
...
(SELECT NVL(O.OFFDAY_LUNCH_BILL,0) FROM HR_TNOFF_ALLOWANCE O
  ...
  AND O.EFFECTIVE_DATE  = (SELECT MAX(EFFECTIVE_DATE) 
         ...
         DEPT_CODE        = Y.DEPT_CODE  AND
         DESIG_CODE      = Y.DESIG_CODE AND
         EFFECTIVE_DATE  <= TO_DATE(:P_MNYR,'MM/YYYY'))) as LUNCH_DIN_ALL,
...
(B.EMP_STATUS IN('A') OR B.EMP_STATUS IN('I') AND 
B.EMP_INACTIVE_DT > (TO_DATE('01/'||:P_MNYR,'DD/MM/YYYY'))) AND
INIT_YR(TO_CHAR(A.ADJ_DATE ,'MM/YYYY')) = INIT_YR(:P_MNYR)   ) X))) Y
